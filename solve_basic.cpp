...
void ServerCluster::solve_basic()
{
    ...
    for(auto& server : servers)
    {
        ...
        auto it = server.assigned_tasks.begin(), end_it = server.assigned_tasks.end();
        while(it != end_it && server.gpu_used > server.capacity)
        {
            size_t task_idx = *it; bool moved = false;
            for(size_t dest_idx = 0; dest_idx < server_num; dest_idx++)
            {
                ...
                // 可以容纳该任务
                if(dest_server.capacity >= tasks[task_idx].demand + dest_server.gpu_used)
                {
                    server.gpu_used -= tasks[task_idx].demand;
                    dest_server.gpu_used += tasks[task_idx].demand;
